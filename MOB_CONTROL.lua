...
UPDATE = 'LATEST'
GLabel = 'MOB_CONTROL'
GProcess = 'com.vincentb.MobControl'
GVersion = 'UPDATE'
xbit = 64 
LibResult = 1
xlib = 'libil2cpp.so'
...
gg.toast(xTAGx)  
mc=gg.multiChoice({
    ED.."REMOVE ENEMY WAVE",
    CD.."UNLOCK/CAN EQUIP CARD", 
	NA.."REMOVE ADS", 
	DU.."RELEASE ULTIMATE",
	ST.."FREE SKIP",
	BL.."BONUS LVL", 
	...
	if mc[1] then deploy() end 
	if mc[2] then cards() end
	if mc[3] then noads() end 
...
-- class VoodooSauce
A[1].method="IsPremiumOrHasOngoingSubscriptions" 

function noads()
if A[1].error==1 then error() return end 
x=1 HACK(1,1,NA) NA=ONOFF(NA) 
end 

--███████████████████████

-- class Shooter
A[2].method="get_CurrentChargeTimePerUnit"  -- float

function champion()
if A[2].error==1 then error() return end 
    if is64 then x={'52800000h','72A88F40h','1E270000h','D65F03C0h'}  -- 1000 f
        else x={'E3000000h','E344047Ah','EE070A90h','EEB00A67h','E12FFF1Eh'}
    end
HACK(2,2,DC) DC=ONOFF(DC) 
end 

--███████████████████████

-- class WinStreakBonusState
A[3].method="get_IsBonusStateActive" 

function winstreak()
if A[3].error==1 then error() return end 
x=1 HACK(3,3,WS) WS=ONOFF(WS) 
end 

--███████████████████████

-- class CardData
for i = 4,8 do A[i].class="CardData" end 
A[4].method="get_IsUsable" 
A[5].method="get_IsLocked" 
A[6].method="get_IsHidden" 
A[7].method="get_RequiredCardsToLevelUp" 
A[8].method="get_LevelPrice" 
...
A[9].method="get_BonusLevelFlag"  -- bool

function bonus()
if A[9].error==1 then error() return end 
x=1 HACK(9,9,BL) BL=ONOFF(BL) 
end 

--███████████████████████

-- class UltimateBar
A[10].method="get_CanTriggerUltimate" 

function ultimate()
if A[10].error==1 then error() return end 
x=1 HACK(10,10,DU) DU=ONOFF(DU) 
end 

--███████████████████████

-- class LuckyBattleCore
A[11].method="get_KeysCost" 

function luckybattle()
if A[11].error==1 then error() return end 
x=0 HACK(11,11,LB) LB=ONOFF(LB) 
end 

--███████████████████████

-- class Cannon
A[12].method="get_ChunkInitialOpponentWaitingTime"  -- float

function deploy()
if A[12].error==1 then error() return end 
    if is64 then x={'52816D60h','72ABABC0h','1E270000h','D65F03C0h'}  -- 1gazillion ~
        else x={'E3000B6Bh','E3450D5Eh','EE070A90h','EEB00A67h','E12FFF1Eh'}
    end
HACK(12,12,ED) ED=ONOFF(ED)
end 

--███████████████████████

-- class Attackable
A[13].method="get_MaxLife"  -- int

function health()
if A[13].error==1 then error() return end 
x=1 HACK(13,13,TH) TH=ONOFF(TH) 
end 

--███████████████████████

-- class SkipetsManager
A[14].method="get_NbSkipetsOwned"  -- long  / edit int 
A[15].class="SkipetsManager"
A[15].method="Consume"  -- void 
...
-- class TruePvpReceivedAttackCell
A[16].class="TruePvpReceivedAttackCell" 
A[16].method="GetAttackCost" 
    ...
    xlibn=0 
        ...
        libx=gg.getRangesList()
            ...
            xsapk={} xsapkx=0
                ...
                if xsapkx~=0 then                 
                OZORAQ=math.max(table.unpack(xsapk))
                    for i, v in ipairs(libx) do              
                        if libx[i].state=="Xa" and libx[i]["end"]-libx[i].start==OZORAQ then              
                        xXx=xXx+1             
                       xOZORAx[xXx]=libx[i].start
                       XLIBSIZE=libx[i]["end"]-libx[i].start 
             ...
             OZORAQ=math.max(table.unpack(xt))
                 for i, v in ipairs(libx) do              
                     if libx[i].state=="Xa" and libx[i]["end"]-libx[i].start==OZORAQ then        
                     xXx=xXx+1       
                     xOZORAx[xXx]=libx[i].start
...
o=tonumber(o) 
    for XxX=1,#(xOZORAx) do
        ...
        cc=0
            for c=1,#(x) do
            xdump[c]={} 
            xdump[c].address=xOZORAx[XxX]+o+cc
            xdump[c].flags=4   
...
function revert()
    ...
    REVERT=nil REVERT={} xRx=nil xRx=1 
        for i, v in ipairs(ORIG) do
            if tonumber(xOZORAx[XxX]+o)==ORIG[i].address then
                if type(x)~="table" then
                    REVERT[xRx]={}
                    REVERT[xRx].address=xOZORAx[XxX]+o
                    REVERT[xRx].flags=4
                    REVERT[xRx].value=ORIG[i].value
                    xRx=xRx+1
                    REVERT[xRx]={}
                    REVERT[xRx].address=xOZORAx[XxX]+o+4
                    REVERT[xRx].flags=4
                ...
                offcnt=0 tnuc=0
                    for dfg=1,#(x) do 
                    REVERT[xRx]={}
                    REVERT[xRx].address=xOZORAx[XxX]+o+offcnt 
                    REVERT[xRx].flags=4
...
xRx=1 ORIG={} xREV={} 
    for XxX=1,#(xOZORAx) do   
        ...
        thiscunt=0
            for dfg=1,7 do 
            ORIG[xRx]={}
            ORIG[xRx].address=xOZORAx[XxX]+tonumber(I[i])+thiscunt 
            ORIG[xRx].flags=4 
            xRx=xRx+1 
            thiscunt=thiscunt+4 
            end 
...
--███████████████████████
while true do
    if gg.isVisible() or ozora==1 then
        gg.setVisible(false) ozora=0 
        menu() 
    end 
end 
